...
{
    public record Discounts(decimal relative)
    {
        public static Discounts Default() => new(0);
    }
    ...
    private readonly Discounts _discounts = Discounts.Default();
    ...
    public BookDetailsModel(ILogger<IndexModel> logger, IUnitOfWork dbContext, IDataSeed<BookPrice> bookPricesSeed,
                            Discounts discounts, ISpecification<Book> spec, IBibliographicEntryFormatter recommendedBooksFormatter) =>
        (_logger, _dbContext, _bookPricesSeed, _discounts, _allBooksSpec, _recommendedBooksFormatter) =
    ...
    private List<PriceLine> CalculatePriceLines(Money originalPrice)
    {
        ...
        _logger.LogInformation("Applying discount: {discount}", _discounts);

        priceLines.Add(new("Original price", originalPrice));

        if (_discounts.relative > 0)
        {
            Money discountAmount = originalPrice * _discounts.relative;
            priceLines.Add(new($"Discount {_discounts.relative:P2}", discountAmount));
        }
